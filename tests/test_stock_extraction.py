...
2026-09-12 사전을 stock_info 로 교체하면서 픽스처를 한 번 더 떴다. 440건 중
9건이 바뀌었고 내역은 전부 다음과 같다.

    추가 7건 — 엔투텍(227950)·지에프아이(493330)×2·한울반도체(320000):
              DART 공시 제목의 회사명. 구 사전에 아예 없던 종목들이다.
              심텍(222800)·효성(004800): 시황·인물 기사의 정상 언급.
              컴투스(078340): 절단 별칭이 덮어써서 사라졌던 종목의 복구.
    제거 2건 — 이닉스(452400): "SK하이닉스 주식"의 꼬리를 규칙5 가 잡던 오탐.
              규칙5 에 앞쪽 경계 검사를 넣어 없앴다.
...
def test_긴_단어의_꼬리는_키워드가_붙어도_종목이_아니다(crawler):
    """규칙5 에 앞쪽 경계 검사가 없어 '신흥국 증권'이 흥국(010240)으로 잡혔다.

    명단과 무관한 기존 결함이라 멀쩡한 이름에도 해당한다.
    """
    assert "010240" not in crawler.extract_stock_codes("신흥국 증권 시장에서 자금 이탈이 이어졌다").split(",")


def test_앞이_경계면_키워드_규칙은_그대로_동작한다(crawler):
    """경계 검사를 넣다가 규칙5 자체를 죽이면 안 된다."""
    assert "005930" in crawler.extract_stock_codes("삼성전자 주가가 올랐다").split(",")
