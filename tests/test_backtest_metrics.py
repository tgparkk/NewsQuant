...
def test_순열은_거래일_안에서만_점수를_섞는다():
    ...
    p_value 비교로는 이 성질을 못 잡는다 — 스피어만은 순위통계라, 어떤
    크기의 부분집합이든 «그 날 자신의 값으로만» 섞든 «전체 풀에서» 섞든
    그 날의 상대순위 분포 자체는 수학적으로 항상 균등무작위가 되어버려
    구별이 안 된다(실측: 몬테카를로 60회, 불균등 그룹 크기 5/20/35/50 +
    날짜별 드리프트를 섞은 귀무가설 데이터에서도 그룹 셔플과 전역 셔플의
    오탐률이 10.0% vs 10.0% 로 정확히 같았다). 그래서 셔플 메커니즘
    자체를 직접 검사한다: 두 거래일의 점수 «수준» 을 완전히 떼어 놓으면
    (day1 은 1000대, day2 는 2000대), 날짜 안에서만 섞을 경우 각 날의
    점수 멀티셋(어떤 값들이 있었는지)은 순서만 바뀌고 그대로 보존돼야
    한다 — 전역으로 섞으면 거의 항상 다른 날의 값이 끼어들어 깨진다."""
    day1, day2 = date(2026, 6, 15), date(2026, 6, 16)
    rows = []
    for i in range(20):
