...
class SentimentAnalyzer:
    ...
    NEGATIVE_KEYWORDS = [
        ...
        # 접두 부정어를 NEGATION_WORDS 에서 뺀 대신 자기 항목으로 등재 (2026-09-11)
        '불가능', '미달', '미흡',
    ...
    #
    # 개선 (2026-09-11): 한 글자 부정어를 어절 내부 부분문자열로 매칭해
    # 긍정 문장을 최대 부정으로 뒤집는 버그를 고쳤다. 실측(수정 전):
    #   '안정적 성장 지속' = -1.000  ('안'정적 이 '성장' 을 반전)
    #   '미국 수출 증가'   = -1.000  ('미'국 이 '증가' 를 반전)
    #   '불확실성 속 매출 증가' = -1.000  ('불'확실성 이 '증가' 를 반전)
    # 한국어에서 부정어가 나타나는 방식이 다르므로 매칭 방식도 나눈다.

    # (a) 어절 전체가 일치해야 하는 부정어 — 독립 부사로 쓰인다.
    # "실적이 안 좋다", "기대에 못 미쳤다" 처럼 어절 그 자체가 부정어다.
    # 부분문자열로 보면 '안정적', '못자리' 가 걸리므로 완전일치만 인정한다.
    NEGATION_WORDS_EXACT = ['안', '못']

    # (b) 어절 안에 부분문자열로 나타나는 부정어 — 어미·활용형으로 붙는다.
    # "없다/없이/없었다", "않다/않았다" 처럼 활용하며 다음절이라 오탐 위험이 낮다.
    NEGATION_WORDS_PARTIAL = [
        '없', '아닌', '않', '꺾',
        '무산', '철회', '취소', '중단', '포기', '실패',
    ]

    # (c) 제거: '불', '미'
    # 이 둘은 뒤 단어를 부정하는 부정어가 아니라 자기 단어의 접두사다
    # (불확실성, 불가능, 미달, 미흡). '미국'·'미래'·'불꽃'·'불구' 를 대량 오탐하고,
    # 자체 부정 카운트 + 뒤 단어 반전으로 이중 계상까지 일으켰다.
    # 해당 단어들은 NEGATIVE_KEYWORDS 에 자기 항목으로 들어 있어 정보 손실이 없다.

    # 하위 호환용 합집합 (외부 참조 대비, 내부 판정에는 쓰지 않는다)
    NEGATION_WORDS = NEGATION_WORDS_EXACT + NEGATION_WORDS_PARTIAL
    ...
    def _check_negation(self, text: str, keyword: str, keyword_pos: int) -> bool:
        ...
        for word in words_to_check:
            # 독립 부사형 부정어는 어절 전체가 일치해야 한다 ('안정적' 오탐 방지)
            if word in self.NEGATION_WORDS_EXACT:
                return True
            # 활용형 부정어는 어절 내부 부분문자열로 매칭한다 ('없는', '않았다')
            for neg in self.NEGATION_WORDS_PARTIAL:
                if neg in word:
                    return True
        return False
